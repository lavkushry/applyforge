    ...
    <Card className="mx-auto max-w-md space-y-6">
      ...
      <form className="space-y-4" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
        <div className="space-y-2">
          ...
          <Input id={emailId} aria-invalid={!!errors.email} aria-describedby={errors.email ? `${emailId}-error` : undefined} {...register("email")} placeholder="you@example.com" />
          {errors.email ? <p id={`${emailId}-error`} role="alert" className="text-xs text-rose-300">{errors.email.message}</p> : null}
        </div>
        <div className="space-y-2">
          <label htmlFor={passwordId} className="text-sm text-slate-300">Password</label>
          <Input id={passwordId} aria-invalid={!!errors.password} aria-describedby={errors.password ? `${passwordId}-error` : undefined} {...register("password")} type="password" placeholder="At least 8 characters" />
          {errors.password ? <p id={`${passwordId}-error`} role="alert" className="text-xs text-rose-300">{errors.password.message}</p> : null}
        </div>

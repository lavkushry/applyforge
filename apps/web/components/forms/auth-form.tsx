    ...
    <Card className="mx-auto max-w-md space-y-6">
      ...
      <form className="space-y-4" onSubmit={handleSubmit((values) => mutation.mutate(values))}>
        <div className="space-y-2">
          ...
          <Input id="email-input" aria-describedby={errors.email ? "email-error" : undefined} {...register("email")} placeholder="you@example.com" />
          {errors.email ? <p id="email-error" role="alert" className="text-xs text-rose-300">{errors.email.message}</p> : null}
        </div>
        <div className="space-y-2">
          <label htmlFor="password-input" className="text-sm text-slate-300">Password</label>
          <Input id="password-input" aria-describedby={errors.password ? "password-error" : undefined} {...register("password")} type="password" placeholder="At least 8 characters" />

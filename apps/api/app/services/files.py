...
def save_upload(filename: str, content: bytes) -> str:
    ...
    # Convert backslashes to forward slashes to prevent traversal, then extract name
    normalized_name = Path(filename.replace("\\", "/")).name
    # Strip any characters not in the allowlist
    safe_name = re.sub(r"[^a-zA-Z0-9_.-]", "", normalized_name)
    if not safe_name:

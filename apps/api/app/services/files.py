...
def save_upload(filename: str, content: bytes) -> str:
    ...
    # SECURE: Normalize backslashes to prevent directory traversal on POSIX systems
    normalized = filename.replace("\\", "/")
    base_name = Path(normalized).name
    # SECURE: Strip unsafe characters
    safe_name = re.sub(r"[^a-zA-Z0-9.\-_]", "_", base_name).lstrip(".-") or "file"

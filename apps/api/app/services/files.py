...
def save_upload(filename: str, content: bytes) -> str:
    ...
    # SECURITY: Manually normalize backslashes to forward slashes because Path(filename).name
    # on POSIX systems does not strip backslashes, preventing path traversal via malicious payloads.
    normalized_filename = filename.replace("\\", "/")
    base_name = Path(normalized_filename).name

    # Strip any characters that aren't alphanumeric, dot, dash, or underscore

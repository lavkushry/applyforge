...
def ingest_target_role(db: Session, user_id: int, role: TargetRole) -> JobIngestionRun:
    ...
    for source in sources:
        ...
        # PRE-FETCH JOBS BY DEDUPE_KEY (BATCHING)
        job_lookup = {}
        if seen_dedupe_keys:
            existing_jobs = (
                db.query(Job).filter(Job.dedupe_key.in_(seen_dedupe_keys)).all()
            )
            for ej in existing_jobs:
                job_lookup[ej.dedupe_key] = ej

        # SECOND PASS TO RESOLVE & CREATE/UPDATE
        for payload in payloads:
            normalized = normalize_job_payload({**payload, "role_id": role.id})

            # RESOLVE COMPANY (Optimization: Try to avoid N+1 DB calls if possible, but keep existing logic for safety first)
            ...
            existing = job_lookup.get(normalized["dedupe_key"])

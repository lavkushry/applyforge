...
def ingest_target_role(db: Session, user_id: int, role: TargetRole) -> JobIngestionRun:
    ...
    for source in sources:
        ...
        for payload in payloads:
            ...
            batch_keys.add(normalized["dedupe_key"])

        existing_jobs_by_key = {}
        if batch_keys:
            for job in db.query(Job).filter(Job.dedupe_key.in_(batch_keys)).all():
                existing_jobs_by_key[job.dedupe_key] = job

        current_target_company = None
        for normalized in normalized_payloads:
            discovered_count += 1
            run.discovered_count = discovered_count
            seen_dedupe_keys.add(normalized["dedupe_key"])

            candidate_name = normalized.get("company", "")
            if (
                current_target_company
                and candidate_name
                and candidate_name.lower() == current_target_company.name.lower()
            ):
                resolved_company = current_target_company
            else:
                ...
                if resolved_company:
                    current_target_company = resolved_company

            normalized["company_id"] = resolved_company.id if resolved_company else None
            existing = existing_jobs_by_key.get(normalized["dedupe_key"])
            ...
            if existing:
                ...
                db.add(job)
                db.flush()
                existing_jobs_by_key[normalized["dedupe_key"]] = job
                inserted_count += 1

...
def ingest_target_role(db: Session, user_id: int, role: TargetRole) -> JobIngestionRun:
    ...
    for source in sources:
        ...
        # ⚡ Bolt: Batch query by dedupe_key to prevent N+1 queries during ingestion
        keys = [p["dedupe_key"] for p in normalized_payloads]
        existing_jobs = (
            {
                job.dedupe_key: job
                for job in db.query(Job).filter(Job.dedupe_key.in_(keys)).all()
            }
            if keys
            else {}
        )

...
def _serialize_role(role: TargetRole, db: Session, cache: dict[int, list[TargetRoleSource]] | None = None) -> TargetRoleOut:
...
def list_roles(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[TargetRoleOut]:
    ...
    # Pre-fetch sources to prevent N+1 query problem

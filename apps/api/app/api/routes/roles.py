...
def list_roles(
    ...
    # Pre-fetch all sources for the roles in a single query to avoid N+1 queries
    sources_cache = {role_id: [] for role_id in role_ids}
    if role_ids:
        sources = (
            db.query(TargetRoleSource)
        ...
        for source in sources:
            sources_cache[source.role_id].append(source)

    return [_serialize_role(role, db, cache=sources_cache) for role in roles]

...
def _serialize_role(
    ...
    if cache and "sources_by_role_id" in cache:
        sources = cache["sources_by_role_id"].get(role.id, [])
    else:
        sources = (
...
def list_roles(
    ...
    # Pre-fetch all role sources to prevent N+1 queries
    role_ids = [role.id for role in roles]
    cache = {"sources_by_role_id": {rid: [] for rid in role_ids}}
    ...
    if role_ids:
        ...
        for source in all_sources:
            cache["sources_by_role_id"][source.role_id].append(source)

...
def _serialize_role(role: TargetRole, db: Session, cache: dict | None = None) -> TargetRoleOut:
    if cache is not None and "sources" in cache and role.id in cache["sources"]:
        sources = cache["sources"][role.id]
    else:
...
def list_roles(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[TargetRoleOut]:
    ...
    cache = {"sources": {r_id: [] for r_id in role_ids}}
    for source in sources:
        cache["sources"][source.role_id].append(source)

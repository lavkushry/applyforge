...
def list_roles(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> list[TargetRoleOut]:
    ...
    # Pre-fetch sources to prevent N+1 queries
    role_ids = [role.id for role in roles]
    sources = db.query(TargetRoleSource).filter(TargetRoleSource.role_id.in_(role_ids)).order_by(TargetRoleSource.id.asc()).all() if role_ids else []

    sources_by_role = {}
    for source in sources:
        sources_by_role.setdefault(source.role_id, []).append(source)

    cache = {"sources": sources_by_role}

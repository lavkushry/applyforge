...
def wizard_summary(
    ...
    # ⚡ Bolt Optimization: Consolidate 6 database queries into a single round-trip using scalar subqueries.
    # Expected Impact: Reduces database latency during setup wizard load by fetching independent counts and existence checks together.
    stmt = select(
        ...
        select(exists().where(Resume.user_id == user.id, Resume.active.is_(True)))
        .scalar_subquery()
        .label("active_resume_exists"),
        select(
            exists().where(
    ...
    basics = result.profile_basics or {}
    skills = result.profile_skills or []
    profile_ready = bool(basics and basics.get("full_name") and skills)
    resume_ready = bool(result.active_resume_exists)

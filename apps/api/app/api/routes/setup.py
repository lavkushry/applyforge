...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    active_resume_sq = select(select(Resume.id).where(Resume.user_id == user.id, Resume.active.is_(True)).exists()).limit(1).scalar_subquery()
    inbox_sq = select(select(InboxConnection.id).where(InboxConnection.user_id == user.id, InboxConnection.status == "connected").exists()).limit(1).scalar_subquery()
    ...
    tailored_count_sq = select(func.count(ResumeVersion.id)).join(Resume, Resume.id == ResumeVersion.resume_id).where(Resume.user_id == user.id).scalar_subquery()

    result = db.execute(
        select(
            profile_basics_sq.label('profile_basics'),
            profile_skills_sq.label('profile_skills'),
            active_resume_sq.label('active_resume'),
            inbox_sq.label('inbox'),
            role_count_sq.label('role_count'),
            job_count_sq.label('job_count'),
            tailored_count_sq.label('tailored_count')
        )
    ).first()

    profile_basics = result.profile_basics or {}
    profile_skills = result.profile_skills or []
    profile_ready = bool(profile_basics.get("full_name") and profile_skills)

    resume_ready = bool(result.active_resume)
    inbox_ready = bool(result.inbox)

    role_count = result.role_count or 0
    job_count = result.job_count or 0
    tailored_resume_count = result.tailored_count or 0

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    # Optimize database calls: consolidate into a single round-trip using scalar_subquery()
    ...
    result = db.execute(
        select(
            ...
            active_resume_sq.label("active_resume_exists"),
            inbox_sq.label("inbox_exists"),

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # ⚡ Bolt: Consolidate 6 independent sequential queries into a single database roundtrip
    # using scalar subqueries to avoid N+1 query patterns and reduce network I/O latency.
    query = select(

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # ⚡ Bolt: Consolidate 6 separate queries into a single round-trip using scalar subqueries
    # Reduces database latency overhead for the summary dashboard
    ...
    query = select(
        ...
        resume_exists_sq.label("resume_exists"),

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # ⚡ Bolt: Consolidate 6 queries into a single database round-trip to improve endpoint response time
    stmt = select(
        ...
        select(select(Resume.id).where(and_(Resume.user_id == user.id, Resume.active.is_(True))).exists()).scalar_subquery().label("resume_ready"),
        select(select(InboxConnection.id).where(and_(InboxConnection.user_id == user.id, InboxConnection.status == "connected")).exists()).scalar_subquery().label("inbox_ready"),
        select(func.count(TargetRole.id)).where(TargetRole.user_id == user.id).scalar_subquery().label("role_count"),
        select(func.count(Job.id)).where(and_(Job.user_id == user.id, Job.active.is_(True))).scalar_subquery().label("job_count"),

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    active_resume_exists_sq = select(select(Resume.id).where(Resume.user_id == user.id, Resume.active.is_(True)).exists()).limit(1).scalar_subquery()
    inbox_exists_sq = select(select(InboxConnection.id).where(InboxConnection.user_id == user.id, InboxConnection.status == "connected").exists()).limit(1).scalar_subquery()
    ...
    tailored_resume_count_sq = select(func.count(ResumeVersion.id)).select_from(ResumeVersion).join(Resume, Resume.id == ResumeVersion.resume_id).where(Resume.user_id == user.id).scalar_subquery()
    ...
    resume_ready = bool(result and result.active_resume_exists)

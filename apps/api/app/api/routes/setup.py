...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # Consolidate metrics and existence checks into a single database round-trip
    # to eliminate N+1 queries using scalar subqueries.
    stmt = select(
        ...
        select(func.count(ResumeVersion.id)).join(Resume, Resume.id == ResumeVersion.resume_id).where(Resume.user_id == user.id).scalar_subquery().label("tailored_resume_count"),
    )
    result = db.execute(stmt).first()

    profile_ready = bool(result and result.profile_basics and result.profile_skills and result.profile_basics.get("full_name"))

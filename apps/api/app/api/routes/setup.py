...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # Consolidate multiple counts and existence checks into a single query via scalar_subquery
    ...
    active_resume_exists_sq = select(
    ...
    stmt = select(
        ...
        active_resume_exists_sq.label("active_resume_exists"),
        inbox_exists_sq.label("inbox_exists"),
    ...
    basics = result.profile_basics if result else None
    skills = result.profile_skills if result else None

    profile_ready = False
    if basics and isinstance(basics, dict) and skills and isinstance(skills, list):
        profile_ready = bool(basics.get("full_name") and skills)

    resume_ready = bool(result.active_resume_exists) if result else False
    inbox_ready = bool(result.inbox_exists) if result else False

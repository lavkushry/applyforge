...
def wizard_summary(
    ...
    # ⚡ Bolt: Consolidated wizard summary queries into a single database round-trip
    # This reduces 6 separate database calls to just 1, significantly improving dashboard load time.
    stmt = select(
    ...
    profile_basics = getattr(row, "profile_basics", None)
    profile_skills = getattr(row, "profile_skills", None)

    profile_ready = False
    if profile_basics and profile_skills:
        profile_ready = bool(profile_basics.get("full_name"))

    resume_ready = getattr(row, "resume_exists", False)
    inbox_ready = getattr(row, "inbox_exists", False)
    role_count = getattr(row, "role_count", 0)
    job_count = getattr(row, "job_count", 0)
    tailored_resume_count = getattr(row, "tailored_resume_count", 0)

...
def wizard_summary(
    ...
    # ⚡ Bolt Optimization: Consolidated 6 separate database queries into a single round-trip
    # using scalar subqueries for improved latency on this dashboard endpoint.
    stmt = select(
        ...
        select(Resume.id)
        .filter(Resume.user_id == user.id, Resume.active.is_(True))
        .limit(1)
        .scalar_subquery()
        .label("active_resume_id"),
        select(InboxConnection.id)
        .filter(
            InboxConnection.user_id == user.id, InboxConnection.status == "connected"
        )
        .order_by(InboxConnection.id.desc())
        .limit(1)
        .scalar_subquery()
        .label("inbox_id"),
        ...
        .filter(Job.user_id == user.id, Job.active.is_(True))
        .scalar_subquery()
        .label("job_count"),
    ...
    resume_ready = bool(result and result.active_resume_id)
    inbox_ready = bool(result and result.inbox_id)
    ...
    tailored_resume_count = (
        result.tailored_resume_count if result and result.tailored_resume_count else 0
    )

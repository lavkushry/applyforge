...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # Consolidate queries into a single database round-trip
    profile_skills_sq = select(CandidateProfile.skills).where(CandidateProfile.user_id == user.id).scalar_subquery()
    profile_basics_sq = select(CandidateProfile.basics).where(CandidateProfile.user_id == user.id).scalar_subquery()

    resume_ready_sq = select(select(Resume.id).where(Resume.user_id == user.id, Resume.active.is_(True)).exists()).scalar_subquery()
    inbox_ready_sq = select(select(InboxConnection.id).where(InboxConnection.user_id == user.id, InboxConnection.status == "connected").exists()).scalar_subquery()
    ...
    query = select(
        ...
        resume_ready_sq.label("resume_ready"),
        inbox_ready_sq.label("inbox_ready"),
    ...
    profile_skills = result.profile_skills if result else None
    profile_basics = result.profile_basics if result else None

    profile_ready = bool(profile_skills and profile_basics and profile_basics.get("full_name"))
    resume_ready = bool(result.resume_ready if result else False)
    inbox_ready = bool(result.inbox_ready if result else False)

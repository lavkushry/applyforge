...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # ⚡ Bolt: Consolidated 6 separate DB queries into a single round-trip using scalar subqueries
    query = select(
        select(CandidateProfile.basics).filter(CandidateProfile.user_id == user.id).limit(1).scalar_subquery().label('profile_basics'),
        select(CandidateProfile.skills).filter(CandidateProfile.user_id == user.id).limit(1).scalar_subquery().label('profile_skills'),
        select(Resume.id).filter(Resume.user_id == user.id, Resume.active.is_(True)).limit(1).scalar_subquery().label('active_resume_id'),
        select(InboxConnection.id).filter(InboxConnection.user_id == user.id, InboxConnection.status == "connected").order_by(InboxConnection.updated_at.desc()).limit(1).scalar_subquery().label('inbox_id'),
        ...
        select(func.count(ResumeVersion.id)).join(Resume, Resume.id == ResumeVersion.resume_id).filter(Resume.user_id == user.id).scalar_subquery().label('tailored_resume_count')
    )
    ...
    resume_ready = bool(stats and stats.active_resume_id)
    inbox_ready = bool(stats and stats.inbox_id)

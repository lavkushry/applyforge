...
def wizard_summary(
    ...
    # ⚡ Bolt Optimization: Consolidate multiple counts and existence checks into a single query round-trip
    # to avoid N+1 query patterns and reduce database latency on the dashboard.
    has_active_resume_q = select(
        exists().where(Resume.user_id == user.id, Resume.active.is_(True))
    ).scalar_subquery()

    has_connected_inbox_q = select(
        exists().where(
    ...
    query = select(
        has_active_resume_q.label("has_active_resume"),
        has_connected_inbox_q.label("has_connected_inbox"),
        role_count_q.label("role_count"),
        job_count_q.label("job_count"),
        tailored_resume_count_q.label("tailored_resume_count"),
    )

    res = db.execute(query).fetchone()
    resume_ready, inbox_ready, role_count, job_count, tailored_resume_count = (
        res if res else (False, False, 0, 0, 0)
    )

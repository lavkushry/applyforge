...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    active_resume_sq = select(select(Resume.id).where(
        Resume.user_id == user.id, Resume.active.is_(True)
    ).exists()).scalar_subquery()

    inbox_sq = select(select(InboxConnection.id).where(
    ...
    tailored_resume_count_sq = select(func.count(ResumeVersion.id)).select_from(ResumeVersion).join(Resume).where(Resume.user_id == user.id).scalar_subquery()

    res = db.execute(
        select(
            profile_basics_sq.label("profile_basics"),
            profile_skills_sq.label("profile_skills"),
            active_resume_sq.label("has_active_resume"),
    ...
    profile_ready = False
    if res and res.profile_basics and res.profile_skills:
        if isinstance(res.profile_basics, dict) and res.profile_basics.get("full_name"):
            profile_ready = True

    resume_ready = bool(res and res.has_active_resume)
    inbox_ready = bool(res and res.has_inbox)
    role_count = res.role_count if res else 0
    job_count = res.job_count if res else 0
    tailored_resume_count = res.tailored_resume_count if res else 0

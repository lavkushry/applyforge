...
def wizard_summary(
    ...
    # ⚡ Bolt Optimization: Consolidate 6 independent queries into a single round trip using scalar subqueries.
    profile_basics_sq = (
    ...
    active_resume_exists_sq = select(
        exists().where(and_(Resume.user_id == user.id, Resume.active.is_(True)))
    ).scalar_subquery()
    ...
    job_count_sq = (
        ...
        .filter(and_(Job.user_id == user.id, Job.active.is_(True)))
        .scalar_subquery()
    )
    ...
    if not result:
        # Fallback values if the database somehow returns None for the single row
        profile_basics = None
        profile_skills = None
        resume_ready = False
        inbox_ready = False

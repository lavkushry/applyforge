...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    ...
    role_count = result.role_count or 0 if result else 0
    job_count = result.job_count or 0 if result else 0
    tailored_resume_count = result.tailored_resume_count or 0 if result else 0

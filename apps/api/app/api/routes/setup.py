...
def wizard_summary(
    ...
    # ⚡ Bolt Optimization: Consolidate 6 separate DB queries into a single round-trip
    summary_query = select(
        ...
        .where(ResumeVersion.resume_id == Resume.id, Resume.user_id == user.id)
        .scalar_subquery()
    ...
    profile_ready = bool(
        result.has_profile and profile_basics.get("full_name") and profile_skills
    )
    resume_ready = bool(result.has_resume)
    inbox_ready = bool(result.has_inbox)

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # ⚡ Bolt: Optimize dashboard summary endpoint
    # Consolidate 6 separate database queries (profile, resume, inbox, role count, job count, tailored resume count)
    # into a single database round-trip using scalar subqueries.
    # This significantly reduces latency on the main dashboard load.
    from sqlalchemy import select, func

    profile_basics = select(CandidateProfile.basics).where(CandidateProfile.user_id == user.id).limit(1).scalar_subquery()
    profile_skills = select(CandidateProfile.skills).where(CandidateProfile.user_id == user.id).limit(1).scalar_subquery()

    active_resume_exists = select(
    ...
    tailored_resume_count_sq = select(func.count(ResumeVersion.id)).join(Resume, Resume.id == ResumeVersion.resume_id).where(Resume.user_id == user.id).scalar_subquery()

    result = db.execute(
        select(
            profile_basics.label("basics"),
            profile_skills.label("skills"),
            active_resume_exists.label("resume_exists"),
            inbox_exists.label("inbox_exists"),
    ...
    resume_ready = bool(result and result.resume_exists)
    inbox_ready = bool(result and result.inbox_exists)

...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # Bolt Optimization: Consolidated 6 individual queries (profile, active resume, inbox, role count, job count, tailored resume count) into a single database round-trip via scalar_subqueries.
    q_profile_basics = select(CandidateProfile.basics).where(CandidateProfile.user_id == user.id).limit(1).scalar_subquery()
    q_profile_skills = select(CandidateProfile.skills).where(CandidateProfile.user_id == user.id).limit(1).scalar_subquery()

    q_active_resume = select(select(Resume.id).where(Resume.user_id == user.id, Resume.active.is_(True)).exists()).scalar_subquery()
    ...
    q_role_count = select(func.count(TargetRole.id)).where(TargetRole.user_id == user.id).scalar_subquery()
    q_job_count = select(func.count(Job.id)).where(Job.user_id == user.id, Job.active.is_(True)).scalar_subquery()

    q_tailored_resume_count = (
    ...
    query = select(
        q_profile_basics.label("profile_basics"),
        q_profile_skills.label("profile_skills"),
        q_active_resume.label("has_active_resume"),
        q_inbox.label("has_inbox"),
        q_role_count.label("role_count"),
        q_job_count.label("job_count"),
        q_tailored_resume_count.label("tailored_resume_count"),
    )

    result = db.execute(query).first()

    profile_ready = bool(
        result and result.profile_basics and
        isinstance(result.profile_basics, dict) and
        result.profile_basics.get("full_name") and
        result.profile_skills
    )
    resume_ready = bool(result and result.has_active_resume)

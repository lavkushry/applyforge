...
def wizard_summary(user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> dict:
    # ⚡ Bolt: Consolidated 6 separate database queries into a single round-trip
    # using scalar subqueries for counts and existence checks.
    ...
    active_resume_exists_sq = select(select(Resume.id).where(Resume.user_id == user.id, Resume.active.is_(True)).exists()).scalar_subquery()
    inbox_exists_sq = select(select(InboxConnection.id).where(InboxConnection.user_id == user.id, InboxConnection.status == "connected").exists()).scalar_subquery()
    ...
    query = select(
        ...
        active_resume_exists_sq.label("resume_ready"),
    ...
    profile_ready = bool(row and row.basics and row.basics.get("full_name") and row.skills)
    resume_ready = bool(row and row.resume_ready)
    inbox_ready = bool(row and row.inbox_ready)
    role_count = row.role_count if row else 0
    job_count = row.job_count if row else 0
    tailored_resume_count = row.tailored_resume_count if row else 0
